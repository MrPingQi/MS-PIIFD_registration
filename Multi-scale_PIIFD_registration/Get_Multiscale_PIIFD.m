...
function descriptors = Get_Multiscale_PIIFD(I,kps,patch_size,NBS,NBO,...
    nOctaves,nLayers,G_resize,G_sigma,int_flag,rot_flag)
...
for octave=1:nOctaves
    ...
    for layer=1:nLayers
        ...
        descriptors{octave,layer} = PIIFD_Descriptor(I_t, ...
            [kps_t(index,1:2),kps(index,1:2)],patch_size,NBS,NBO,int_flag,rot_flag);
    end
end
